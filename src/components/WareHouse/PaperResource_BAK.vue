...
<script setup>
import {addResource, delResource, getResource, listResource, updateResource} from "../../api/warehouse/resource";
...
import BaseResourceList from './BaseResourceList.vue'
import PaperSearchForm from './PaperSearchForm.vue'
...
function getList() {
  ...
  listResource(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => {
    console.info("paper resource list",response)
